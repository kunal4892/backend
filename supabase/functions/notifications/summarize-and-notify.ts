// supabase/functions/summarize-and-notify/index.ts
...
//   const prompt = `You are ${personaName}, a fun and engaging persona in a chat app.
// Summarize the last 30 messages into a message notification (max 20 words).
// The teaser should feel natural, personal, and make the user want to continue the chat.
// Do NOT say "summary". Just write a casual hook in Hinglish like a message from a friend.`;
...
//   return data?.candidates?.[0]?.content?.parts?.map((p)=>p.text).join("") || "Hey, missing you… come back! 😉";
// }
// serve(async ()=>{
//   try {
//     // 1️⃣ Get all users
...
//       // 4️⃣ Get persona name
//       // const { data: persona } = await supabase.from("personas").select("name").eq("id", thread.persona_id).single();
//       const personaName = thread.persona_id || "AI Friend";
//       // 5️⃣ Generate teaser summary
